...
@view_config(
    ...
    renderer="osmhm_site:templates/object_watch.mako",
...
def object_watch(request):
    try:
        ...
        history = (
            DBSession.query(History_Objects, Watched_Objects)
            .filter(Watched_Objects.authorid == userid)
            .join(Watched_Objects, History_Objects.wid == Watched_Objects.id)
            .order_by(desc(History_Objects.changeset))
            .all()
        )
...
@view_config(
    ...
    renderer="osmhm_site:templates/object_watch_list.mako",
    permission="watch_user_or_object",
)
def object_watch_list(request):
    try:
        userid = authenticated_userid(request)
        objects = (
            DBSession.query(Watched_Objects)
            .filter(Watched_Objects.authorid == userid)
            .all()
        )
    except DBAPIError:
...
@view_config(
    ...
    renderer="osmhm_site:templates/object_watch_list_add.mako",
