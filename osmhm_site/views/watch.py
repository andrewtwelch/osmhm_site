...
def watch(request):
    try:
        history = (
            DBSession.query(History_Filters)
            .order_by(desc(History_Filters.changeset))
            .all()
        )
...
@view_config(
    ...
    renderer="osmhm_site:templates/watch_whitelist.mako",
...
@view_config(
    ...
    renderer="osmhm_site:templates/admin_whitelist_add.mako",
    permission="edit_user_or_object",
)
