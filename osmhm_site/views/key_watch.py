...
@view_config(
    ...
    renderer="osmhm_site:templates/key_watch.mako",
...
def key_watch(request):
    try:
        ...
        full_history = (
            DBSession.query(History_Keys)
            .filter(Watched_Keys.authorid == userid)
            .join(Watched_Keys, History_Keys.wid == Watched_Keys.id)
            .order_by(desc(History_Keys.changeset))
            .all()
        )
        history = (
            DBSession.query(History_Keys)
            .filter(Watched_Keys.authorid == userid)
            .join(Watched_Keys, History_Keys.wid == Watched_Keys.id)
            .distinct(History_Keys.username, History_Keys.key, History_Keys.value)
            .all()
        )
        ...
        for event in full_history:
            ...
            if (
                event.changeset
                not in changesets[event.username][event.key][event.value]
            ):
                changesets[event.username][event.key][event.value].append(
                    event.changeset
                )
                changeset_strs[event.username][event.key][event.value] += (
                    '<a href="https://www.openstreetmap.org/changeset/%s" target="_blank">%s</a> (%s), '
                    % (str(event.changeset), str(event.changeset), str(event.timestamp))
                )
            objects[event.username][event.key][event.value] += 1

        for event in history:
            event.changeset_count = len(
                changesets[event.username][event.key][event.value]
            )
...
def key_watch_event_delete(request):
    DBSession.query(History_Keys).filter_by(
...
@view_config(
    ...
    renderer="osmhm_site:templates/key_watch_list.mako",
    permission="watch_user_or_object",
)
def key_watch_list(request):
    try:
        userid = authenticated_userid(request)
        keys = (
            DBSession.query(Watched_Keys).filter(Watched_Keys.authorid == userid).all()
        )
    except DBAPIError:
        print("Sorry")
    if not keys:
...
@view_config(
    ...
    renderer="osmhm_site:templates/key_watch_list_add.mako",
